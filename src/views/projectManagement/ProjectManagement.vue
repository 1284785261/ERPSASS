      ...
      <div class="industry-box-form">
        <el-form ref="form" class="filter-container">
          <el-form-item label="项目名称" label-width="80px">
            <el-input v-model.trim="searchObject.Name" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入项目名称" @keyup.enter.native="search" />
          </el-form-item>
          <el-form-item label="负责人名称" label-width="110px">
            <el-input v-model.trim="searchObject.SysUserName" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入负责人名称" @keyup.enter.native="search" />
          </el-form-item>
          <el-form-item label="项目业务类型" label-width="110px">
            <el-select v-model.trim="searchObject.BusinessType" clearable placeholder="请选择项目业务类型" size="mini" class="select-input" @keyup.enter.native="search">
          ...
          <el-form-item label="项目优先级" label-width="100px">
            <el-select v-model.trim="searchObject.Priority" clearable placeholder="请选择项目优先级" size="mini" class="select-input" @keyup.enter.native="search">
      ...
      <div>
        ...
        <div class="roletable-box">
          <el-table ref="industrtTable" v-loading="tableloading" :data="ProjectList" class="erp-table" max-height="600" style="width: 100%;margin-bottom: 20px;" tooltip-effect="dark" @selection-change="handleSelectionChange">
            ...
            <el-table-column align="center" prop="Code" min-width="140" label="项目编码" />
            ...
            <el-table-column align="center" prop="BusinessTypeText" min-width="100" label="项目类型" />
            <el-table-column align="center" prop="Priority" min-width="120" label="项目优先级">
              <template slot-scope="scope">
                <span>{{scope.row.Priority | Priority}}</span>
              </template>
            </el-table-column>
            <el-table-column align="center" prop="OwnerName" min-width="120" label="项目跟单员" />
            <el-table-column align="center" v-if="tabidx =='0'" prop="ProgressText" min-width="120" label="项目跟单进度" />
            <el-table-column align="center" v-if="tabidx >'-1'" prop="AdminName" min-width="100" label="项目管理员" show-overflow-tooltip />
            <el-table-column align="center" prop="CompanyName" label="相关客户" min-width="150" show-overflow-tooltip />
            <el-table-column align="center" prop="AddTime" min-width="120" label="项目新建日期">
              <template slot-scope="scope">
                <span>{{scope.row.AddTime | Time}}</span>
              </template>
            </el-table-column>
            <el-table-column align="center" prop="FollowCount" min-width="120" label="项目沟通记录">
              <template slot-scope="scope">
                <span>{{scope.row.FollowCount}}条</span>
              </template>
            </el-table-column>
            <el-table-column align="center" prop="StatusText" min-width="100" label="项目状态" />
            <el-table-column align="center" v-if="tabidx =='1'" prop="ProgressText" min-width="120" label="转交审核状态" />
            <el-table-column align="center" prop="SysUserName" min-width="100" label="项目创建人" />
...
import {
  ...
  AddProjectAssign,
  AddProjectTenderApply,
  GetProjectList,
  GetProjectBusinessType,
  EditProjectStatus,
  AddProjectFollow,
  EditProjectProgress,
  AddProjectBuzCommision,
...
export default {
  ...
  data() {
    return {
      showReferralServiceDepartment: false, //转交服务部门弹窗
      responsiblePersonId: "",
      tableloading: false,
      searchObject: {
        Name: "",
  ...
  methods: {
    ...
    getProjectList() {
      ...
      const obj = {
        ...
        BusinessType:
          this.searchObject.BusinessType > "-1"
            ? this.searchObject.BusinessType
            : "-1",
        Priority:
          this.searchObject.Priority > "-1" ? this.searchObject.Priority : "-1",
        Status: this.tabidx,
      ...
      if (value == 3) {
        ...
        GetProject(qs.stringify(objTwo)).then(res => {
          if (res.IsSuccess) {
            ...
            let ISMarketType =
              this.projectform.MarketType > "-1" ? true : false;
            let ISFirstMonthIncome = this.projectform.FirstMonthIncome
              ? true
              : false;
            let ISFirstMonthNum = this.projectform.FirstMonthNum ? true : false;
            let ISFirstMonthMgrFee = this.projectform.FirstMonthMgrFee
              ? true
              : false;
            if (
              !ISMgrPrice ||
