    ...
    <div class="search-form">
      <el-form ref="form" class="filter-container">
          <el-form-item label="所属客户" label-width="80px" v-if="!IsSpending">
            <el-input v-model="searchform.CompanyName" size="small" clearable class="erpsearchinput" suffix-icon="el-icon-search" placeholder="请输入所属客户查询" />
          </el-form-item>
          <el-form-item label="所属项目" label-width="80px"  v-if="!IsSpending">
            <el-input v-model="searchform.ProjectName" size="small" disabled clearable class="erpsearchinput" suffix-icon="el-icon-search" placeholder="请输入所属项目查询" />
          </el-form-item>
    ...
    <div class="content">
      ...
      <div class="operate-table">
        ...
        <el-table ref="multipleTable" class="erp-table" v-loading="tableloading" v-if="IsSpending" :data="SpendingList" max-height="600" tooltip-effect="dark">
          ...
          <el-table-column align="center" prop="StatusText" v-if="tabidx==0" label="状态" min-width="180" show-overflow-tooltip />
...
import AddSpend from './AddSpend.vue'
import qs from 'qs'
...
export default {
  ...
  data () {
    return {
      ...
      SpendingList: [],
      operates: [
        {MenuId: 106, OperationCode: "Add", OperationId: 1, OperationName: "添加",Status: false},
        {MenuId: 106, OperationCode: "Revoke", OperationId: 93, OperationName: "撤销",Status: false},
        {MenuId: 106, OperationCode: "Modify", OperationId: 2, OperationName: "修改",Status: false},
        {MenuId: 106, OperationCode: "View", OperationId: 4, OperationName: "查看",Status: false},
        {MenuId: 106, OperationCode: "Query", OperationId: 16, OperationName: "查询",Status: false},
        {MenuId: 106, OperationCode: "AddBuyerExpend", OperationId: 78, OperationName: "添加挂账支出",Status: false},
      ], // 获取页面权限
      operatiOBj: { // 父权限查看数据
        menuid: '',
        OperationCode: '1'
  ...
  created () {
    ...
    this.searchform.ProjectName= projectdetail.Name
    console.log(projectdetail)
    // if (this.$route.meta.operations) {
    //   this.operates = this.$route.meta.operations
    //   this.operatiOBj = this.$operatedata(this.operates, 'Query')
    // }
