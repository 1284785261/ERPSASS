        ...
        <el-row :gutter="64">
          ...
          <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            <el-form-item label="所属客户" prop="CompanyId">
              ...
              <el-select v-model="dataObject.CompanyId" disabled filterable clearable remote placeholder="请查询或下拉选择所属客户" class="select-input" @clear="clearCompany" :loading="loading" @change="SelectCustom(dataObject.CompanyId)">
          ...
          <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            <el-form-item label="所属项目" prop="ProjectId">
              <el-select v-model="dataObject.ProjectId" disabled filterable clearable remote placeholder="请先查询客户再选择所属项目" class="select-input" @clear="clearProject" :remote-method="remoteProject" :loading="loading" @change="selectProject(dataObject.ProjectId)">
          ...
          <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            <el-form-item label="回款流水" prop="ProjectMoneyDrawId">
              <el-select v-model="dataObject.ProjectMoneyDrawId" filterable clearable placeholder=" " class="select-input" :loading="loading">
                <el-option v-for="item in RelatedCapitalFlowList" :key="item.Id" :label="'流水金额 '+item.Amount+' 元 - '+'打款账号'+'[ '+item.CompanyAccount+' ]'" :value="item.Id" />
              </el-select>
            </el-form-item>
          </el-col>
            ...
            <el-form-item>
              <div>
                <div class="colors">
                  ...
                  当前项目您需要抵扣的代垫金额为 {{dataObject.DeductionAmount}} 元
                </div>
              </div>
            </el-form-item>
          </el-col>
  ...
  watch: {
    ...
    showAddCard(newvalue, oldvalue) {
      ...
      if (this.dialogTitle == "修改挂账") {
        ...
        this.dataObject.YearMonth = this.dataObject.Year + "-" + this.dataObject.Month;
      } else {
        this.dataObject = this.$options.data().dataObject;
      }
      let projectdetail = JSON.parse(window.localStorage.getItem("projectdetail")) || {};
      if (projectdetail) {
        ...
        this.SelectCustom(this.dataObject.CompanyId)
        this.selectProject(this.dataObject.ProjectId)
      }
    }
          ...
          if (response.Data && response.Data.length > 0) {
            response.Data.forEach(item => {
              if (item.Id == Number(Ids)) {
                ...
                this.dataObject.ProjectId = item.Id;
                this.CompanyPageList = [
                  {
                ...
                this.dataObject.CompanyId = item.CompanyId;
                // this.SelectCustom(this.dataObject.CompanyId);
              }
            });
