...
<template>
    <div>
        ...
        <el-popover placement="bottom-start"  :width="popoverWidth"    trigger="manual"  v-model="isShowSelect" @hide="popoverHide">
            <el-tree class="common-tree erpscroll" :width="width"   ref="tree" :data="treeData" :props="obj"
            ...
            <el-row>
                <el-button v-if="multiple" class="erbBtn" @click="isShowSelect=false" size="small" type="primary">确定</el-button>
            </el-row>
        </el-popover>
    </div>
</template>
            ...
            nodeClick (data, node) {
               if (!this.multiple) {//单选
                   ...
                   for(let i=0;i<checkedKeys.length;i++){
                       for(let j=i+1;j<checkedKeys.length+1;j++){
                   ...
                   this.returnDataKeys = t.map((item) => {
                           return item.userId;
                   });
