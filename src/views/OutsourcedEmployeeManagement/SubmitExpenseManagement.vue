    ...
    <div class="content">
      ...
      <div class="operate-table" v-if="!ShowclaimList">
        <el-table ref="multipleTable" class="erp-table" key="table1" v-loading="tableloading" :data="MoneyRecordList" max-height="600" tooltip-effect="dark" @selection-change="handleSelectionChange">
          ...
          <el-table-column align="center" v-if="$getColumsFlag('Id',checkboxVal)" prop="Id" min-width="80" label="序号" />
          ...
          <el-table-column align="center" v-if="$getColumsFlag('Account',checkboxVal)" prop="Account" label="收款银行账号" min-width="150" />
              ...
              <el-dropdown trigger="click">
                ...
                <el-dropdown-menu slot="dropdown">
                  <ul class="popout-list">
                    <el-dropdown-item v-if="$operatebtn(operates, 'ClaimSubsidies')" @click.native="LookClaimSituation(scope.row,'ClaimSubsidies')">认领补贴回款</el-dropdown-item>
                    <el-dropdown-item v-if="$operatebtn(operates, 'View')" @click.native="LookClaimStatus(scope.row,'View')">查看认领情况</el-dropdown-item>
                  </ul>
                </el-dropdown-menu>
              </el-dropdown>
            </template>
      ...
      <div class="operate-table" v-if="ShowclaimList">
        <el-table ref="multipleTable" class="erp-table" key="table2" v-loading="tableloading" :data="ClaimSituationList" max-height="600" tooltip-effect="dark">
          ...
          <el-table-column align="center" prop="ProjectName" label="认领项目" />
          <el-table-column align="center" prop="MoneyRecordAmount" label="流水金额">
            <template slot-scope="scope">
              <span>{{scope.row.Amount | MoneyUnit}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column align="center" prop="SysUserName" label="领款人" />
      ...
      <!-- 认领补贴回款 -->
      <ClaimSubsidy :isShow="isShow" @isHide="isHide"></ClaimSubsidy>
      <!-- 添加回款 -->
...
<script>
import { GetMoneyRecordPageList, AddMoneyRecord, EditMoneyRecord, GetMoneyRecordInfo, GetMoneyRecordExcelModel, ImportMoneyRecordExcel, DeleteMoneyRecord } from '@/api/FinancialManagement.js'
import { GetProjectMoneyDrawPageList } from '@/api/projectManagement.js'
import ClaimSubsidy from './components/ClaimSubsidy.vue'
import ScreeningComponent from '@/components/publicComponents/ScreeningComponent'
// import AddCollection from '../FinancialManagement/components/AddCollection.vue'
  ...
  methods: {
    ...
    getMoneyRecord () {
      ...
      let data = {
        ...
        MoneyRecordType: 2,
        Status: this.MoneyRecordTypeIdx,
        menuid: MenuIdDate(),
        operationcode: '1'
      }
    ...
    searchClaimSituation () {
      let obj = {
        ...
        MoneyRecordId: this.searchform.MoneyRecordId,
        menuid: MenuIdDate(),
        operationcode: '1'
      }
      this.tableloading = true
      GetProjectMoneyDrawPageList(qs.stringify(obj)).then(response => {
        if (response.IsSuccess) {
          if (response.Data && response.Data.length > 0) {
    ...
    //查看认领情况
    LookClaimStatus (data, str) {
      const { href } = this.$router.resolve({
        path: '/OutsourcedEmployeeManagement/ViewClaims',
        query: {
          name: '申请购买招标文件',
        }
      })
