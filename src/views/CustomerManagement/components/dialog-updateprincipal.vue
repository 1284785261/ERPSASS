      ...
      <div class="tree-box">
        ...
        <div class="choose-box">
          <el-input
            v-model="SysUserName"
            size="small"
            placeholder="请输入查询您要修改的负责人名称"
            @keyup.enter.native="chooseSys"
            @blur="chooseSys"
          ></el-input>
          <el-tree
            :data="newarray"
        ...
        <div class="project-box">
          <div class="tip" v-if="radio.name">{{radio.name}}
            <svg-icon class="cursor-pointer" @click="deleteuser()" slot="suffix" icon-class="Close" class-name="way" />
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
      ...
      if (index == undefined) {
        ...
        for (let i = 0; i < array.length; i++) {
          ...
          if (array[i].children && array[i].children.length) {
            this.changetree(newarray[i], i, array[i].children);
          } else if (array[i].SysOrgList && array[i].SysOrgList.length) {
        ...
        for (let i = 0; i < array.length; i++) {
          if (newarray) {
            ...
            if (array[i].children && array[i].children.length) {
              ...
              for (let j = 0; j < array[i].SysOrgList.length; j++) {
                newarray.children[i].children.push({
                  name: array[i].SysOrgList[j].SysUserName,
                  Id: array[i].SysOrgList[j].SysUserId,
                  children: []
                });
    ...
    save() {
      ...
      if (this.radio) {
        this.$emit("saveprincipal", this.radio.userId);
      } else {
        this.$message({
          message: "请选择要修改的同事",
          type: "warning"
