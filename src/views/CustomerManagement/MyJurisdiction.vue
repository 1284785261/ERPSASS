  ...
  <div class="operate-container">
    ...
    <div class="content">
      ...
      <div class="operate-table">
        <el-table ref="multipleTable" class="erp-table" v-loading="tableloading" :data="tableData" tooltip-effect="dark" max-height="600" @selection-change="handleSelectionChange">
          <el-table-column align="center" type="selection" width="55" fixed="left" label="全选" />
          <el-table-column align="center" v-if="$getColumsFlag('Code',checkboxVal)" prop="Code" min-width="150" fixed="left" label="编号" />
          <el-table-column align="center" v-if="$getColumsFlag('Name',checkboxVal)" prop="Name" min-width="180" fixed="left" label="客户名称" show-overflow-tooltip>
            <template slot-scope="scope">
              <span class="watchcode" @click="watchcustom(scope.row, 'View')">{{scope.row.Name}}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" v-if="$getColumsFlag('BusinessTypeArr',checkboxVal)" prop="BusinessTypeArr" min-width="100" label="业务类型" show-overflow-tooltip />
          <el-table-column align="center" v-if="$getColumsFlag('LevelText',checkboxVal)" prop="LevelText" min-width="100" label="客户等级" />
          <el-table-column align="center" v-if="$getColumsFlag('SysUserName',checkboxVal)" prop="SysUserName" min-width="80" label="负责人" />
          <el-table-column align="center" v-if="$getColumsFlag('TypeText',checkboxVal)" prop="TypeText" min-width="100" label="客户状态" />
          <el-table-column align="center" v-if="$getColumsFlag('NCCode',checkboxVal)" prop="NCCode" min-width="180" label="NC码" />
          <el-table-column align="center" v-if="$getColumsFlag('AddTime',checkboxVal)" prop="AddTime" min-width="160" label="报备时间">
            <template slot-scope="scope">
              {{scope.row.AddTime | YYMMddhhmmssTime}}
            </template>
          </el-table-column>
          <el-table-column align="center" v-if="$getColumsFlag('FollowDate',checkboxVal)" prop="FollowDate" min-width="160" label="最近跟进时间">
            <template slot-scope="scope">
              {{scope.row.FollowDate | YYMMddhhmmssTime}}
            </template>
          </el-table-column>
          <el-table-column align="center" v-if="$getColumsFlag('ProjectNumber',checkboxVal)" prop="ProjectNumber" min-width="120" label="相关项目数量" />
          <el-table-column align="center" v-if="$getColumsFlag('FollowNumber',checkboxVal)" prop="FollowNumber" min-width="120" label="跟进信息数量">
            <template slot-scope="scope">
              <span class="watchcode" @click="watchfollow(scope.row, 'View')">{{scope.row.FollowNumber}}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" v-if="$getColumsFlag('Status',checkboxVal)" prop="Status" min-width="100" label="使用状态">
            <template slot-scope="scope">
      ...
      <div class="operate-bottom">
        ...
        <!-- <el-button size="small" class="addmenu erpbtn" @click="sharecustomer">转为共享客户</el-button> -->
    ...
    <div class="dialog">
      ...
      <!-- <dialogsharecustomer :title="sharename" :visible="sharevisible" :contentdata="sharecontentdata" @closedialog="closedialog"></dialogsharecustomer> -->
    </div>
    <!-- 添加提醒 -->
...
import { GetCompanyPageList, AddCompanyFollow, EditCompanyStatus, EditCompanyType, EditCompanySysUserId } from '@/api/CustomerManagement.js'
