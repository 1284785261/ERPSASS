  ...
  <div class="ContractTemplate-Container">
    <div class="search-bar clearfix">
      ...
      <div class="condition">
        ...
        <el-select v-model="searchform.TempType" class="erpsearchinput" size="small" clearable placeholder="请选择合同类型">
      ...
      <div class="condition">
        ...
        <el-select v-model="searchform.BusinessType" class="erpsearchinput" size="small" clearable placeholder="请选择业务类型">
    ...
    <div class="content">
      ...
      <div class="operate-table">
        <el-table ref="multipleTable" class="erp-table" v-loading="tableloading" :data="tableData" max-height="600" tooltip-effect="dark" @selection-change="handleSelectionChange">
          ...
          <el-table-column align="center" prop="TempTypeText" label="合同类型" min-width="120" />
              ...
              <el-dropdown trigger="click" v-if="($operatebtns(operates,['Modify','Delete','Down','View','Enabled','Disabled']))">
                ...
                <el-dropdown-menu slot="dropdown">
                  <ul class="popout-list">
                    <el-dropdown-item v-if="$operatebtn(operates, 'Modify')" @click.native="EditContract(scope.row,'Modify')">修改</el-dropdown-item>
                    ...
                    <el-dropdown-item v-if="scope.row.Status == 2 && $operatebtn(operates, 'Enabled')" @click.native="EditContractStatus(scope.row,'Enabled')">启用</el-dropdown-item>
                    <el-dropdown-item v-if="scope.row.Status == 1 && $operatebtn(operates, 'Disabled')" @click.native="EditContractStatus(scope.row,'Disabled')">禁用</el-dropdown-item>
                    <el-dropdown-item v-if="$operatebtn(operates, 'Delete')" @click.native="DelectContract(scope.row,'Delete')">删除</el-dropdown-item>
                  </ul>
                </el-dropdown-menu>
              </el-dropdown>
            </template>
    ...
    <div>
      <!-- 新建修改合同模板 -->
      <EditContract :showEditContract="showEditContract" :title="EditContractTitle" :dataContract="dataContract" @EditContractSave="EditContractSave" @CloseEditContract="CloseEditContract"></EditContract>
      <!-- 预览弹窗 -->
...
import { GetContractTemplateEnum, GetContractTemplatePageList, AddContractTemplate1, DelContractTemplate1, EditContractTemplate, BatchDownLoadContractTemplate } from '@/api/FileTemplateManagement.js'
...
export default {
  ...
  data () {
    return {
      ...
      EditContractTitle: '新建合同模板',
      showOperationLog: false,
      OperationLogTitle: '操作日志详情',
      dataContract: {},//修改的数据
      OperationLogType: '1',//1 合同模板   2 证明模板   3 原始模板
      OperationLogId: '',
      isImg: 1,
      previewFileUrl: '',
      titlename: '合同模板预览',
  ...
  methods: {
    ...
    EditContractSave (data) {
      ...
      if (data.Id) {
        ...
        showLoading()
        EditContractTemplate(qs.stringify(param)).then(response => {
          hideLoading()
          if (response.IsSuccess) {
            this.$message({
              type: 'success',
        ...
        showLoading()
        AddContractTemplate1(qs.stringify(param)).then(response => {
          hideLoading()
          if (response.IsSuccess) {
            this.$message({
              type: 'success',
    ...
    DelectContract (data, str) {
      ...
      this.$confirm('此操作将删除该合同模板', '提示', {
        ...
        showLoading()
        DelContractTemplate1(qs.stringify(obj)).then(response => {
          hideLoading()
          if (response.IsSuccess) {
            this.$message({
              type: 'success',
    ...
    //修改合同模板
    EditContract (data, str) {
      this.EditContractTitle = '编辑合同模板'
    ...
    //关闭合同模板弹窗
    CloseEditContract (str) {
      this.showEditContract = str
    },
    //关闭操作日志弹窗
    ...
    //修改状态
    EditContractStatus (data, str) {
      const operating = this.$operatedata(this.operates, str)
      let param = {
        Id: data.Id,
        Name: data.Name,
        TempType: data.TempTypeValue,
        BusinessType: data.BusinessTypeValue,
        Status: '',
        Attachment: data.Attachment,
        menuid: operating.MenuId,
        operationcode: operating.OperationCode,
      ...
      if (data.Status == '1') {
        param.Status = '2'
        strPrompt = '你确定禁用当前合同模板吗?'
        successPrompt = '禁用当前合同模板成功！'
      } else if (data.Status == '2') {
        param.Status = '1'
        strPrompt = '你确定启用当前合同模板吗?'
        successPrompt = '启用当前合同模板成功！'
      }
    ...
    MultipleDownload (str) {
      ...
      showLoading()
      BatchDownLoadContractTemplate(qs.stringify(param)).then(response => {
        hideLoading()
