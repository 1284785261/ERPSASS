    ...
    <el-dialog :close-on-click-modal="false" :title="title" class="dialog-box" append-to-body :visible.sync="showOperation" :show-close="false" :destroy-on-close="true" width="1000px" @close="close">
      ...
      <div class="search-bar clearfix">
        ...
        <div class="condition">
          ...
          <el-date-picker v-model="searchform.periodOf" class="period" size="small" value-format="yyyy-MM-dd HH:mm:ss" type="datetimerange" start-placeholder="开始月份" end-placeholder="结束月份">
          </el-date-picker>
        </div>
        <div class="condition">
      ...
      <div class="content">
        <div class="operate-table">
          <el-table ref="multipleTable" class="erp-table" v-loading="tableloading" :data="tableData" max-height="600" tooltip-effect="dark">
            ...
            <el-table-column align="center" v-if="this.searchform.Flag != 3" prop="BusinessTypeText" label="业务类型" min-width="120" />
...
export default {
  ...
  watch: {
    showOperationLog (newvalue, oldvalue) {
      ...
      this.searchform.Flag = this.type
      this.getPageList()
    }
  },
  created () {
  ...
  methods: {
    ...
    getPageList () {
      let data = {
        ...
        SysUser: this.searchform.SysUser,
      ...
      if (this.searchform.periodOf && this.searchform.periodOf.length > 0) {
        data.BeginTime = this.searchform.periodOf[0]
