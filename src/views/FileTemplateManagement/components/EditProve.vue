...
<template>
  <div class="tag-container">
    <el-dialog :close-on-click-modal="false" :title="title" class="dialog-box" append-to-body :visible.sync="showProve" :show-close="false" :destroy-on-close="true" width="480px" @close="close">
      ...
      <el-form ref="ruleForm" :model="dataObject" :rules="Formrules" class="forms-box">
        ...
        <el-form-item label="证明类型" prop="TempType">
          <el-select v-model="dataObject.TempType" filterable clearable placeholder="请选择证明类型" class="select-input">
        ...
        <el-form-item label="证明模板文件" class="attachment" prop="Attachment">
          <div class="upload">
...
export default {
  name: 'addContract',
  props: ['showEditProve', 'title', 'dataProve'],
  data () {
    const _this = this
    return {
      dataObject: {
      ...
      Formrules: {
        ...
        Attachment: [
          { required: true, message: '请上传证明模板文件', trigger: 'change' }
        ],
      }
    }
  },
  watch: {
    showEditProve (newvalue, oldvalue) {
      this.showProve = newvalue
      this.dataObject = this.$options.data().dataObject
      this.GetContractTemplateEnum()
      if (this.title == '编辑证明模板') {
        this.dataObject = {
          Id: this.dataProve.Id,
          Name: this.dataProve.Name,
          TempType: this.dataProve.TempTypeValue,
          BusinessType: this.dataProve.BusinessTypeValue,
          Status: this.dataProve.Status + '',
          Attachment: this.dataProve.Attachment,
        }
        this.fileName = this.dataProve.Attachment
        this.filestype = this.dataProve.Attachment.slice(this.dataProve.Attachment.lastIndexOf('.') + 1).toUpperCase()
      } else {
