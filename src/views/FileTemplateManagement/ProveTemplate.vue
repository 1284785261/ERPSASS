...
<template>
  <div class="ContractTemplate-Container">
    <div class="search-bar clearfix">
      ...
      <div class="condition">
        <div class="search-name">证明类型</div>
        <el-select v-model="searchform.TempType" class="erpsearchinput" size="small" clearable placeholder="请选择证明类型">
    ...
    <div class="content">
      ...
      <div class="operate-table">
        <el-table ref="multipleTable" class="erp-table" v-loading="tableloading" :data="tableData" max-height="600" tooltip-effect="dark" @selection-change="handleSelectionChange">
          ...
          <el-table-column align="center" prop="TempTypeText" label="证明类型" min-width="120" />
              ...
              <el-dropdown trigger="click" v-if="($operatebtns(operates,['Modify','Delete','Down','View','Enabled','Disabled']))">
                ...
                <el-dropdown-menu slot="dropdown">
                  <ul class="popout-list">
                    <el-dropdown-item v-if="$operatebtn(operates, 'Modify')" @click.native="EditProve(scope.row,'Modify')">修改</el-dropdown-item>
                    ...
                    <el-dropdown-item v-if="scope.row.Status == 2 && $operatebtn(operates, 'Enabled')" @click.native="EditProveStatus(scope.row,'Enabled')">启用</el-dropdown-item>
                    <el-dropdown-item v-if="scope.row.Status == 1 && $operatebtn(operates, 'Disabled')" @click.native="EditProveStatus(scope.row,'Disabled')">禁用</el-dropdown-item>
    ...
    <div>
      <!-- 新建修改证明模板 -->
      <EditProve :showEditProve="showEditProve" :title="EditProveTitle" :dataProve="dataProve" @EditContractSave="EditContractSave" @CloseEditContract="CloseEditContract"></EditProve>
      <!-- 预览弹窗 -->
...
<script>
import EditProve from './components/EditProve'
import OperationLog from './components/OperationLog'
import PreviewFile from '@/components/publicComponents/PreviewFile.vue'
import { GetContractTemplateEnum, GetProveTemplatePageList, AddProveTemplate1, DelProvetTemplate1, EditProveTemplate, BatchDownLoadProveTemplate } from '@/api/FileTemplateManagement.js'
...
export default {
  ...
  data () {
    return {
      ...
      allchecked: false, // 全选
      showEditProve: false,
      EditProveTitle: '新建证明模板',
      showOperationLog: false,
      OperationLogTitle: '操作日志详情',
      dataProve: {},//修改的数据
      OperationLogType: '2',//1 合同模板   2 证明模板   3 原始模板
      OperationLogId: '',
      isImg: 1,
      previewFileUrl: '',
      titlename: '证明模板预览',
  ...
  methods: {
    ...
    GetContract (operat) {
      ...
      GetProveTemplatePageList(qs.stringify(obj)).then(response => {
    ...
    EditContractSave (data) {
      ...
      if (data.Id) {
        ...
        showLoading()
        EditProveTemplate(qs.stringify(param)).then(response => {
          hideLoading()
          if (response.IsSuccess) {
            this.$message({
              type: 'success',
        ...
        showLoading()
        AddProveTemplate1(qs.stringify(param)).then(response => {
          hideLoading()
          if (response.IsSuccess) {
            this.$message({
              type: 'success',
    ...
    DelectProve (data, str) {
      ...
      this.$confirm('此操作将删除该证明模板', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        showLoading()
        DelProvetTemplate1(qs.stringify(obj)).then(response => {
          hideLoading()
          if (response.IsSuccess) {
            this.$message({
              type: 'success',
    ...
    EditProveStatus (data, str) {
      ...
      if (data.Status == '1') {
        param.Status = '2'
        strPrompt = '你确定禁用当前证明模板吗?'
        successPrompt = '禁用当前证明模板成功！'
      } else if (data.Status == '2') {
        param.Status = '1'
        strPrompt = '你确定启用当前证明模板吗?'
        successPrompt = '启用当前证明模板成功！'
      }
    ...
    MultipleDownload (str) {
      ...
      showLoading()
      BatchDownLoadProveTemplate(qs.stringify(param)).then(response => {
        hideLoading()
