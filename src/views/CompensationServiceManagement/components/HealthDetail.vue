    ...
    <div class="final-statement-content">
      <div class="search-bar clearfix" v-show="tabidx==1">
        ...
        <div class="condition">
          ...
          <el-input class="erpsearchinput" placeholder="请输入企业医保账号进行查询" size="small" v-model="searchform.bifAccount" clearable @keyup.enter.native="searchtable">
          </el-input>
        </div>
        <div class="condition">
      ...
      <el-table ref="finalTable" v-if="tabidx==0 && Type==1" key="1" v-loading="tableloading" :data="tableData" :span-method="objectSpanMethod" class="erp-table" tooltip-effect="dark">
        ...
        <el-table-column align="center" prop="Account" label="医保账号" min-width="150" show-overflow-tooltip />
        ...
        <el-table-column align="center" prop="BigMedicalEnt" label="企业部分大病险" min-width="150" />
        <el-table-column align="center" label="企业部分医保费用小计" min-width="180">
          <template slot-scope="scope">
            <span>{{ scope.row.MedicalEnt + scope.row.BigMedicalEnt + scope.row.PayBackEnt - scope.row.DiffPayEnt | KeepTwo }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="MedicalPers" label="个人部分医疗险" min-width="150" />
        <el-table-column align="center" prop="BigMedicalPers" label="个人部分大病险" min-width="150" />
        <el-table-column align="center" label="个人部分医保费用小计" min-width="180">
          <template slot-scope="scope">
            <span>{{ scope.row.MedicalPers + scope.row.BigMedicalPers + scope.row.PayBackPers - scope.row.DiffPayPers | KeepTwo }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="医保缴纳总计" min-width="150">
          <template slot-scope="scope">
            <span>{{scope.row.TotalMedical | KeepTwo}}</span>
          </template>
        </el-table-column>
      ...
      <div class="pagina-fonter" v-if="tabidx==0">
      ...
      <p v-if="tabidx==1">结算人员医保总计：{{SettlementTotal | KeepTwo}}元</p>
      ...
      <!-- <div class="pagina-fonter" v-if="tabidx==1">
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="MembercurrentPage" :page-sizes="[10, 20, 50, 100]" :page-size="MemberPageSize" layout="total, sizes, prev, pager, next, jumper" :total="Membertotal"></el-pagination>
      </div> -->
...
<script>
import { GetMedicalSettleDetailList, GetMedicalSettleMemberList } from '@/api/CompensationServiceManagement.js'
...
export default {
  ...
  created () {
    ...
    this.getMedicalSettleDetailList()
  },
  methods: {
    // 处理合计
    getSummaries (param) {
    ...
    choosetab (e) {
      ...
      if (this.tabidx == e.value) {
        ...
        if (e.value === 0) {
          ...
          this.getMedicalSettleMemberList()
        }
      }
    },
    // 公积金明细
    getMedicalSettleDetailList () {
      let obj = {
        MedicalSettleId: this.Id,
      ...
      GetMedicalSettleDetailList(qs.stringify(obj)).then(response => {
    ...
    // 公积金员工
    getMedicalSettleMemberList () {
      let obj = {
        MedicalSettleId: this.Id,
        PageIndex: this.MembercurrentPage,
      ...
      GetMedicalSettleMemberList(qs.stringify(obj)).then(response => {
        if (response.IsSuccess) {
