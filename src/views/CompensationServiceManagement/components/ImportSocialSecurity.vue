...
<template>
  <div class="tag-container">
    <el-dialog :close-on-click-modal="false" title="导入社保" class="dialog-box" append-to-body :visible.sync="showSocialSecurity" :show-close="false" :destroy-on-close="true" width="800px" @close="close">
      <i class="el-icon-circle-close closeButton" @click="close"></i>
      <el-form ref="ruleForm" :model="dataObject" :rules="rules" class="forms">
        <el-form-item label="选择要导入的表格" clearable prop="InsuranceUrl" class="File-item">
          ...
          <el-input v-model="dataObject.InsuranceUrl" disabled size="small" />
        </el-form-item>
        ...
        <el-form-item label="所属年月所在列" prop="SiPeriod">
          <el-input v-model="dataObject.SiPeriod" clearable size="small" placeholder="请输入所属年月所在列" />
        </el-form-item>
        <el-form-item label="险种排列方式" prop="SiType" class="State">
          <el-radio-group v-model="dataObject.SiType">
            <el-radio label="0">纵向</el-radio>
            <el-radio label="1">横向</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="险种类型所在列" prop="SiTypeIndex" v-if="dataObject.SiType == '0'">
          <el-input v-model="dataObject.SiTypeIndex" clearable size="small" placeholder="请输入险种类型所在列" />
        </el-form-item>
        <el-form-item label="缴费类型所在列" prop="PayType" v-if="dataObject.SiType == '0'">
        ...
        <el-form-item label="身份证/个人编号所在列" prop="SiCode">
          <el-input v-model="dataObject.SiCode" clearable size="small" placeholder="请输入身份证/个人编号所在列" />
        </el-form-item>
        <el-form-item label="社保个人编号所在列">
          <el-input v-model="dataObject.AccountColumn" clearable size="small" placeholder="请输入社保个人编号所在列" />
        </el-form-item>
      </el-form>
...
import { GetInsuranceExcelModel, GetBfInsuranceFundPageList, GetInsuranceBatchInfoByBfInsuranceFundId } from '@/api/CompensationServiceManagement.js'
  ...
  methods: {
      ...
      if (files.size > (20 * 1024 * 1024)) {
        ...
        uploader.UploadFun(() => {
          if (uploader.urlResult) {
            ...
            this.dataObject.InsuranceUrl = uploader.urlResult.VideoUrl
    ...
    SelectValue (val) {
      if (val) {
        ...
        GetInsuranceBatchInfoByBfInsuranceFundId(qs.stringify(obj)).then(response => {
          if (response.IsSuccess && response.Data) {
            Object.keys(this.dataObject).forEach(key => {
              if (key != 'BfInsuranceFundId' && key != 'InsuranceUrl' && key != 'PeriodYearMonth' && key != 'file') {
                this.dataObject[key] = response.Data[key]
              }
            })
            this.dataObject.SiType += ''
