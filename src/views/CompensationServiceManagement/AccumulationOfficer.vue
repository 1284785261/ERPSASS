<template>
  <div class="accumulation-officer-container">
    <div class="search-bar clearfix">
      ...
      <div class="condition">
        <div class="search-name">公积金打款账号</div>
        <el-input class="erpsearchinput" clearable placeholder="请输入公积金打款账号查询" size="small" v-model="searchform.Account">
        </el-input>
      </div>
      <div class="condition">
    ...
    <div class="tab2 clearfix">
      ...
      <!-- <el-button size="small" class="erpbtn" @click="addpersonnel">公积金增减人员</el-button> -->
    </div>
    <div class="content">
      <div class="operate-table">
        <el-table ref="multipleTable" class="erp-table" v-loading="tableloading" :data="StaffChangeList" max-height="600" tooltip-effect="dark" @selection-change="handleSelectionChange">
          ...
          <el-table-column align="center" prop="Account" label="公积金打款账号" min-width="150" show-overflow-tooltip />
          <el-table-column v-if="tabidx===0" align="center" prop="BeginYear" label="缴纳起始月份" min-width="120">
            <template slot-scope="scope">
              <span v-if="scope.row.BeginYear">{{scope.row.BeginYear}}年{{scope.row.BeginMonth}}月</span>
              <span v-else></span>
            </template>
          </el-table-column>
          <el-table-column v-if="tabidx===1" align="center" prop="EndYear" label="缴纳截止月份" min-width="120">
            <template slot-scope="scope">
...
import { GetHouseFundChangePageList, DeleteHouseFundChange, EditHouseFundChange, DownloadHouseFundChangeExcel } from '@/api/CompensationServiceManagement.js'
...
export default {
  ...
  methods: {
    ...
    getTabelList (operat) {
      ...
      GetHouseFundChangePageList(qs.stringify(obj)).then(response => {
    ...
    saverevision (e) {
      ...
      showLoading()
      EditHouseFundChange(qs.stringify(obj)).then(response => {
        hideLoading()
        if (response.IsSuccess) {
          this.$message({
            message: '修改成功',
    ...
    removepersonnel (e) {
      ...
      let message = ''
      // if (this.tabidx === 0) {
      //   message = `移除成功！（检测到该员工的公积金缴纳状态为未缴纳时）
      //   当前客户已执行增员操作，请前往减员名单进行处理！（检测到该员工的公积金缴纳状态为已缴纳时）`
      //   title = '从增员名单移除'
      // } else if (this.tabidx === 1) {
      //   message = '移除成功！（检测到该员工的公积金缴纳状态为缴纳中时）'
      //   title = '从增员名单移除'
      // }
      ...
      // return
      showLoading()
      DeleteHouseFundChange(qs.stringify(obj)).then(response => {
        hideLoading()
        if (response.IsSuccess) {
          let title = ''
    ...
    OpenStatements () {
      ...
      if (this.Ids.length) {
        ...
        showLoading()
        DownloadHouseFundChangeExcel(qs.stringify(obj)).then(response => {
          hideLoading()
          if (response.IsSuccess) {
            if (response.Data) {
...
.accumulation-officer-container {
  .search-bar {
