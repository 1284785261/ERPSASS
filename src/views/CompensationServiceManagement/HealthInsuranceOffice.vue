  ...
  <div class="social-security-officer-container">
    <div class="search-bar clearfix">
      <div class="condition">
        ...
        <el-input class="erpsearchinput" clearable placeholder="请输入员工名称查询" size="small" v-model.trim="searchform.StaffName" @keyup.enter.native="searchtable">
          <i class="el-icon-search el-input__icon" slot="suffix" @click="searchtable" />
        </el-input>
      </div>
      <div class="condition">
        <div class="search-name">缴交起始年月</div>
        <el-date-picker class="erpsearchinput" clearable v-model="searchform.YearMonth" size="small" type="date" placeholder="选择日期" @keyup.enter.native="searchtable"></el-date-picker>
      </div>
      <!-- <div class="condition">
      ...
      <div class="condition">
        <div class="search-name">医保打款账号</div>
        <el-input class="erpsearchinput" clearable placeholder="请输入医保打款账号查询" size="small" v-model.trim="searchform.Account" @keyup.enter.native="searchtable">
        </el-input>
      </div>
      <div class="condition">
        <div class="search-name">处理人</div>
        <el-input class="erpsearchinput" clearable placeholder="请输入处理人查询" size="small" v-model.trim="searchform.UserName" @keyup.enter.native="searchtable">
        </el-input>
      </div>
      <div class="condition">
    ...
    <div class="content">
      <div class="operate-table">
        <el-table ref="multipleTable" class="erp-table" v-loading="tableloading" :data="StaffChangeList" max-height="600" tooltip-effect="dark" @selection-change="handleSelectionChange">
          ...
          <el-table-column align="center" prop="Account" label="医保打款账号" min-width="150" show-overflow-tooltip />
          <el-table-column v-if="tabidx===0" key="1" align="center" prop="BeginYear" label="缴纳起始年月" min-width="120">
            <template slot-scope="scope">
              <span v-if="scope.row.BeginYear">{{scope.row.BeginYear}}年{{scope.row.BeginMonth}}月</span>
              <span v-else></span>
            </template>
          </el-table-column>
          <el-table-column v-if="tabidx===1" key="2" align="center" prop="EndYear" label="缴纳截止年月" min-width="120">
            <template slot-scope="scope">
              <span v-if="scope.row.EndYear">{{scope.row.EndYear}}年{{scope.row.EndMonth}}月</span>
              <span v-else></span>
            </template>
          </el-table-column>
          <el-table-column v-if="tabidx===0" key="3" align="center" prop="AddUserName" label="处理人" min-width="120" />
          <el-table-column v-if="tabidx===1" key="4" align="center" prop="ReduceUserName" label="处理人" min-width="120" />
          <el-table-column v-if="tabidx===0" key="5" align="center" prop="AddTime" label="处理时间" min-width="120" show-overflow-tooltip>
            <template slot-scope="scope">
              <span v-if="scope.row.AddTime">{{scope.row.AddTime | Time}}</span>
              <span v-else></span>
            </template>
          </el-table-column>
          <el-table-column v-if="tabidx===1" key="6" align="center" prop="ReduceTime" label="处理时间" min-width="120">
            <template slot-scope="scope">
              <span v-if="scope.row.ReduceTime">{{scope.row.ReduceTime | Time}}</span>
              <span v-else></span>
            </template>
          </el-table-column>
...
import { GetMedicalChangePageList, EditMedicalChange, DeleteMedicalChange, DownloadMedicalChangeExcel } from '@/api/CompensationServiceManagement.js'
...
export default {
  ...
  methods: {
    ...
    getTabelList (operat) {
      ...
      GetMedicalChangePageList(qs.stringify(obj)).then(response => {
    ...
    saverevision (e) {
      ...
      showLoading()
      EditMedicalChange(qs.stringify(obj)).then(response => {
        hideLoading()
        if (response.IsSuccess) {
          this.$message({
            message: '修改成功',
    ...
    removepersonnel (e, str) {
      ...
      // });
      showLoading()
      DeleteMedicalChange(qs.stringify(obj)).then(response => {
        hideLoading()
        if (response.IsSuccess) {
          let title = ''
    ...
    OpenStatements () {
      ...
      if (this.Ids.length) {
        ...
        showLoading()
        DownloadMedicalChangeExcel(qs.stringify(obj)).then(response => {
          hideLoading()
          if (response.IsSuccess) {
            if (response.Data) {
