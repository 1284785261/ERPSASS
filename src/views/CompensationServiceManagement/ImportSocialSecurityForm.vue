<!-- 导入社保表格页面 -->
<template>
  <div class="ImportSocialSecurityForm">
    <div class="bread">
      <BreadCrumb :url-list="urlList" />
    </div>
    <div class="ImportSocialSecurityForm-content">
      <div class="description-box">
        <el-steps :active="currentActive" align-center>
          <el-step title="导入基础信息" description="请选择项目以及录入导入的表格的基础信息"></el-step>
          <el-step title="选择社保表及数据区域" description="请选择你需要导入系统的sheet表格"></el-step>
      ...
      <div v-if="currentActive == 0">
        <el-form ref="ruleForm" :model="OnedataObject" :rules="rules" class="OneForms">
          ...
          <el-form-item label="企业社保账号" prop="BfInsuranceFundId" class="File-item">
            <el-select v-model="OnedataObject.BfInsuranceFundId" filterable clearable remote placeholder="请查询或下拉选择企业社保账号" class="select-input" @clear="clearProject" @change="SelectValue" :remote-method="remoteInsurance" :loading="loading">
      ...
      <div v-if="currentActive == 2">
        <div slot="footer" class="buttons-top">
          <el-button round size="small" @click="AddSpareLate()">{{AddLateTtile}}</el-button>
          <el-button round size="small" @click="AddSpareInterest()">{{AddInterestTtile}}</el-button>
          <el-button round size="small" @click="AddSpareBack()">{{AddBackTtile}}</el-button>
          <el-button round size="small" @click="AddSparePayment()">{{AddPaymentTtile}}</el-button>
        </div>
      ...
      <div v-if="currentActive == 3">
        <div class="FourArea">
          <div style="margin: 20px 0 20px 30px;color:#FF4A4A;">导入社保成功 {{SuccessesNum}} 条，失败 {{ErrorDNum}} 条！</div>
          <div v-if="showData">
...
import { GetInsuranceExcelModel, GetBfInsuranceFundPageList, GetSalaryExcelDataStruct, GetImportToExcelOrErpStructuralData, ImportToExcelOrErpMapping, GetInsuranceBatchInfoByBfInsuranceFundId } from '@/api/CompensationServiceManagement.js'
...
export default {
  ...
  data () {
    return {
      ...
      AddLateTtile: '添加滞纳金',
      AddInterest: true,
      AddInterestTtile: '添加利息',
      AddBack: true,
      AddBackTtile: '添加退回',
      AddPayment: true,
      AddPaymentTtile: '添加补缴',
      //第四步
      errorData: [],
      showData: true,
      SuccessesNum: 0,
  ...
  methods: {
    ...
    dialogtrue2 () {
      ...
      excelObj.append('SiType', this.DangerPlanted)//险种列
      excelObj.append('SiTypeIndex', this.TwoFrom.SiTypeIndex)
      ...
      excelObj.append('SiCode', this.TwoFrom.SiCode)
      excelObj.append('SiPeriod', this.TwoFrom.SiPeriod)
      ...
      GetImportToExcelOrErpStructuralData(excelObj).then(response => {
        ...
        if (response && response.ErpStructuralData && response.ImportToExcelStructuralData) {
          ...
          this.ErpStructuraArr = [
            { 'title': '基础信息', 'ListArr': [] },
            { 'title': '企业部分社保', 'ListArr': [] },
            { 'title': '个人部分社保', 'ListArr': [] },
            { 'title': '备注', 'ListArr': [] }
          ]
          for (let i = 0; i < this.headerData.ErpStructuralData.length; i++) {
            ...
            if (this.headerData.ErpStructuralData[i] == '社保所属年月') {
              this.ErpStructuraArr[0].ListArr.push({ 'stytitle': this.headerData.ErpStructuralData[i], 'header': '' })
            }
          ...
          GetInsuranceBatchInfoByBfInsuranceFundId(qs.stringify(backfillObj)).then(response => {
            if (response.IsSuccess && response.Data) {
              if (response.Data.Remark) {
                ...
                for (let i = 0; i < arrs.length; i++) {
                  if (arrs[i].ERPName.indexOf(LateTtile) != -1) {
                    IsLate = true
                  }
                  if (arrs[i].ERPName.indexOf(InterestTtile) != -1) {
                    IsInterest = true
                  }
                  if (arrs[i].ERPName.indexOf(BackTtile) != -1) {
                    IsBack = true
                  }
                  if (arrs[i].ERPName.indexOf(PaymentTtile) != -1) {
                    IsPayment = true
                  }
                }
                //添加滞纳金
                if (IsLate) {
                  this.AddLate = false
                  this.AddLateTtile = '删除滞纳金'
                  this.ErpStructuraArr[1].ListArr.push(
                    { 'stytitle': '养老保险滞纳金企业部分', 'header': '' },
                    { 'stytitle': '医疗保险滞纳金企业部分', 'header': '' },
                    { 'stytitle': '大额保险滞纳金企业部分', 'header': '' },
                    { 'stytitle': '失业保险滞纳金企业部分', 'header': '' },
                    { 'stytitle': '生育保险滞纳金企业部分', 'header': '' },
                    { 'stytitle': '工伤保险滞纳金企业部分', 'header': '' }
                  )
                  this.ErpStructuraArr[2].ListArr.push(
                    { 'stytitle': '养老保险滞纳金个人部分', 'header': '' },
                    { 'stytitle': '医疗保险滞纳金个人部分', 'header': '' },
                    { 'stytitle': '大额保险滞纳金个人部分', 'header': '' },
                    { 'stytitle': '失业保险滞纳金个人部分', 'header': '' },
                    { 'stytitle': '生育保险滞纳金个人部分', 'header': '' },
                    { 'stytitle': '工伤保险滞纳金个人部分', 'header': '' }
                  )
                }
                //添加利息
                if (IsInterest) {
                  this.AddInterest = false
                  this.AddInterestTtile = '删除利息'
                  this.ErpStructuraArr[1].ListArr.push(
                    { 'stytitle': '养老保险利息企业部分', 'header': '' },
                    { 'stytitle': '医疗保险利息企业部分', 'header': '' },
                    { 'stytitle': '大额保险利息企业部分', 'header': '' },
                    { 'stytitle': '失业保险利息企业部分', 'header': '' },
                    { 'stytitle': '生育保险利息企业部分', 'header': '' },
                    { 'stytitle': '工伤保险利息企业部分', 'header': '' }
                  )
                  this.ErpStructuraArr[2].ListArr.push(
                    { 'stytitle': '养老保险利息个人部分', 'header': '' },
                    { 'stytitle': '医疗保险利息个人部分', 'header': '' },
                    { 'stytitle': '大额保险利息个人部分', 'header': '' },
                    { 'stytitle': '失业保险利息个人部分', 'header': '' },
                    { 'stytitle': '生育保险利息个人部分', 'header': '' },
                    { 'stytitle': '工伤保险利息个人部分', 'header': '' }
                  )
                }
                //添加退回
                if (IsBack) {
                  this.AddBack = false
                  this.AddBackTtile = '删除退回'
                  this.ErpStructuraArr[1].ListArr.push(
                    { 'stytitle': '养老保险企业退回', 'header': '' },
                    { 'stytitle': '医疗保险企业退回', 'header': '' },
                    { 'stytitle': '大额保险企业退回', 'header': '' },
                    { 'stytitle': '失业保险企业退回', 'header': '' },
                    { 'stytitle': '生育保险企业退回', 'header': '' },
                    { 'stytitle': '工伤保险企业退回', 'header': '' }
                  )
                  this.ErpStructuraArr[2].ListArr.push(
                    { 'stytitle': '养老保险个人退回', 'header': '' },
                    { 'stytitle': '医疗保险个人退回', 'header': '' },
                    { 'stytitle': '大额保险个人退回', 'header': '' },
                    { 'stytitle': '失业保险个人退回', 'header': '' },
                    { 'stytitle': '生育保险个人退回', 'header': '' },
                    { 'stytitle': '工伤保险个人退回', 'header': '' }
                  )
                }
                //添加补缴
                if (IsPayment) {
                  this.AddPayment = false
                  this.AddPaymentTtile = '删除补缴'
                  this.ErpStructuraArr[1].ListArr.push(
                    { 'stytitle': '养老保险企业补缴', 'header': '' },
                    { 'stytitle': '医疗保险企业补缴', 'header': '' },
                    { 'stytitle': '大额保险企业补缴', 'header': '' },
                    { 'stytitle': '失业保险企业补缴', 'header': '' },
                    { 'stytitle': '生育保险企业补缴', 'header': '' },
                    { 'stytitle': '工伤保险企业补缴', 'header': '' }
                  )
                  this.ErpStructuraArr[2].ListArr.push(
                    { 'stytitle': '养老保险个人补缴', 'header': '' },
                    { 'stytitle': '医疗保险个人补缴', 'header': '' },
                    { 'stytitle': '大额保险个人补缴', 'header': '' },
                    { 'stytitle': '失业保险个人补缴', 'header': '' },
                    { 'stytitle': '生育保险个人补缴', 'header': '' },
                    { 'stytitle': '工伤保险个人补缴', 'header': '' }
                  )
                }
    ...
    //添加滞纳金
    AddSpareLate () {
      this.AddLate = !this.AddLate
      if (this.AddLate) {
        this.AddLateTtile = '添加滞纳金'
        let str = '滞纳金'
        for (let i = 0; i < this.ErpStructuraArr[1].ListArr.length; i++) {
          if (this.ErpStructuraArr[1].ListArr[i].stytitle.indexOf(str) != -1) {
            this.ErpStructuraArr[1].ListArr.splice(i, 1)
            i--
          }
        }
        for (let i = 0; i < this.ErpStructuraArr[2].ListArr.length; i++) {
          if (this.ErpStructuraArr[2].ListArr[i].stytitle.indexOf(str) != -1) {
            this.ErpStructuraArr[2].ListArr.splice(i, 1)
            i--
          }
        }
      } else {
    ...
    //添加退回
    AddSpareBack () {
      this.AddBack = !this.AddBack
      if (this.AddBack) {
        this.AddBackTtile = '添加退回'
        let str = '退回'
        for (let i = 0; i < this.ErpStructuraArr[1].ListArr.length; i++) {
          if (this.ErpStructuraArr[1].ListArr[i].stytitle.indexOf(str) != -1) {
            this.ErpStructuraArr[1].ListArr.splice(i, 1)
    ...
    dialogtrue3 () {
      ...
      excelObj.append('SiType', this.DangerPlanted)
      excelObj.append('SiTypeIndex', this.TwoFrom.SiTypeIndex)
      excelObj.append('PayType', this.TwoFrom.PayType)
      excelObj.append('PayerType', this.PayObject)
      excelObj.append('PayerIndex', this.TwoFrom.PayerIndex)
      excelObj.append('SiCode', this.TwoFrom.SiCode)
      excelObj.append('SiPeriod', this.TwoFrom.SiPeriod)
      excelObj.append('InsuranceUrl', this.OnedataObject.InsuranceUrl)
    ...
    retrunList () {
      this.$router.push({
        path: '/CompensationServiceManagement/SocialSecurityManagement',
        query: {
        }
      })
    },
...
.ImportSocialSecurityForm {
  .bread {
    margin-left: 24px;
    margin-bottom: 24px;
  }
  .ImportSocialSecurityForm-content {
    background: #fff;
    min-height: 700px;
    .description-box {
      padding: 20px 0;
