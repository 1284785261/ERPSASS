<!-- 导入公积金 -->
<template>
  <div class="ImportProvidentFundForm">
    <div class="bread">
      <BreadCrumb :url-list="urlList" />
    </div>
    <div class="ImportProvidentFundForm-content">
      <div class="description-box">
        <el-steps :active="currentActive" align-center>
          <el-step title="导入基础信息" description="请选择项目以及录入导入的表格的基础信息"></el-step>
          <el-step title="选择公积金表及数据区域" description="请选择你需要导入系统的sheet表格"></el-step>
      ...
      <div v-if="currentActive == 0">
        <el-form ref="ruleForm" :model="OnedataObject" :rules="rules" class="OneForms">
          ...
          <el-form-item label="企业公积金账户" prop="BfInsuranceFundId" class="File-item">
            <el-select v-model="OnedataObject.BfInsuranceFundId" filterable clearable remote placeholder="请查询或下拉选择企业公积金账户" class="select-input" @clear="clearProject" :remote-method="remoteInsurance" :loading="loading">
          ...
          <el-form-item label="所属年月" prop="YearMonth" class="date">
            <el-date-picker v-model="OnedataObject.YearMonth" value-format="yyyy-MM" clearable type="month" placeholder="请选择所属年月" />
          </el-form-item>
        </el-form>
      ...
      <div v-if="currentActive == 3">
        <div class="FourArea">
          <div style="margin: 20px 0 20px 30px;color:#FF4A4A;">导入公积金成功 {{SuccessesNum}} 条，失败 {{ErrorDNum}} 条！</div>
          <div v-if="showData">
...
import { GetImportHouseFundToExcelOrErpStructuralData, GetBfInsuranceFundPageList, GetSalaryExcelDataStruct, GetHouseFundBatchInfoByBfId, ImportHouseFundToExcelOrErpMapping } from '@/api/CompensationServiceManagement.js'
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        BfInsuranceFundId: [
          { required: true, message: '请选择公司公积金账号', trigger: 'change' }
        ],
        PeriodYearMonth: [
  ...
  methods: {
    ...
    dialogtrue2 () {
      ...
      excelObj.append('IdCodeIndex', this.TwoFrom.SiCode)
      ...
      GetImportHouseFundToExcelOrErpStructuralData(excelObj).then(response => {
        ...
        if (response && response.ErpStructuralData && response.ImportToExcelStructuralData) {
          ...
          this.ErpStructuraArr = [
            { 'title': '基础信息', 'ListArr': [] },
            { 'title': '企业部分公积金', 'ListArr': [] },
            { 'title': '个人部分公积金', 'ListArr': [] },
            { 'title': '备注', 'ListArr': [] }
          ]
          for (let i = 0; i < this.headerData.ErpStructuralData.length; i++) {
            if (this.headerData.ErpStructuralData[i] == '缴交基数') {
              this.ErpStructuraArr[0].ListArr.push({ 'stytitle': this.headerData.ErpStructuralData[i], 'header': '' })
            }
            if (this.headerData.ErpStructuralData[i] == '账户余额') {
              this.ErpStructuraArr[0].ListArr.push({ 'stytitle': this.headerData.ErpStructuralData[i], 'header': '' })
            }
            if (this.headerData.ErpStructuralData[i] == '滞纳金') {
              this.ErpStructuraArr[0].ListArr.push({ 'stytitle': this.headerData.ErpStructuralData[i], 'header': '' })
            }
          ...
          GetHouseFundBatchInfoByBfId(qs.stringify(backfillObj)).then(response => {
            if (response.IsSuccess && response.Data) {
              console.log(response, this.ErpStructuraArr)
    ...
    dialogtrue3 () {
      ...
      excelObj.append('IdCodeIndex', this.TwoFrom.SiCode)
      excelObj.append('HouseFundUrl', this.OnedataObject.InsuranceUrl)
    ...
    retrunList () {
      this.$router.push({
        path: '/CompensationServiceManagement/ProvidentFundManagement',
        query: {
        }
      })
    },
...
.ImportProvidentFundForm {
  .bread {
    margin-left: 24px;
    margin-bottom: 24px;
  }
  .ImportProvidentFundForm-content {
    background: #fff;
    min-height: 700px;
    .description-box {
      padding: 20px 0;
