  ...
  <div class="social-security-management-container">
      ...
      <div class="industry-box-form">
        <el-form ref="form" class="filter-container">
          <el-form-item label="账号" label-width="50px">
            <el-input v-model.trim="searchform.Account" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入账号" @keyup.enter.native="searchtable" />
          </el-form-item>
          <el-form-item label="开户公司主体" label-width="120px">
            <el-input v-model.trim="searchform.BfCompanyName" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入开户公司主体" @keyup.enter.native="searchtable" />
          </el-form-item>
          <el-form-item label="所属客户" label-width="80px">
            <el-input v-model.trim="searchform.CompanyName" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入所属客户" @keyup.enter.native="searchtable" />
          </el-form-item>
          <el-form-item label="开户机构" label-width="80px">
            <el-input v-model.trim="searchform.Institution" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入开户机构" @keyup.enter.native="searchtable" />
          </el-form-item>
          <el-form-item label="账号负责人" label-width="100px">
            <el-input v-model.trim="searchform.ContactPerson" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入账号负责人" @keyup.enter.native="searchtable" />
          </el-form-item>
    ...
    <div class="content">
      <div class="operate-top">
        <el-button size="small" v-if="$operatebtn(operates, 'Add')" class="erpbtn" @click="OpenAccount('Add')">添加医保账号</el-button>
      ...
      <div class="operate-table">
        <el-table ref="multipleTable" class="erp-table" v-loading="tableloading" :data="BfInsuranceList" max-height="600" tooltip-effect="dark" @selection-change="handleSelectionChange">
          ...
          <el-table-column align="center" v-if="$getColumsFlag('Tel',checkboxVal)" prop="Tel" label="开户机构电话" min-width="180" />
...
export default {
  ...
  methods: {
    ...
    UploadStatus (data, str) {
      ...
      if (data.Status == '1') {
        param.Status = '0'
        strPrompt = '你确定禁用此医保账户吗?'
        successPrompt = '禁用医保账户成功！'
      } else if (data.Status == '0') {
        param.Status = '1'
        strPrompt = '你确定启用此医保账户吗?'
        successPrompt = '启用医保账户成功！'
      }
