  ...
  <div class="social-security-management-container">
      ...
      <div class="industry-box-form">
        <el-form ref="form" class="filter-container">
          <el-form-item label="账号" label-width="60px">
            <el-input v-model="searchform.Account" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入账号进行查询" />
          </el-form-item>
          <el-form-item label="开户公司主体" label-width="110px">
            <el-input v-model="searchform.BfCompanyName" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入开户公司主体进行查询" />
          </el-form-item>
          <el-form-item label="所属客户" label-width="80px">
            <el-input v-model="searchform.CompanyName" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入所属客户进行查询" />
          </el-form-item>
          <el-form-item label="开户机构" label-width="80px">
            <el-input v-model="searchform.Institution" clearable class="select-input" suffix-icon="el-icon-search" placeholder="请输入开户机构进行查询" />
          </el-form-item>
    ...
    <div class="content">
      <div class="operate-top">
        <el-button size="small" v-if="$operatebtn(operates, 'Add')" class="erpbtn" @click="OpenAccount('Add')">添加社保账号</el-button>
      ...
      <div class="operate-table">
        <el-table ref="multipleTable" class="erp-table" v-loading="tableloading" :data="BfInsuranceList" max-height="600" tooltip-effect="dark" @selection-change="handleSelectionChange">
          ...
          <el-table-column align="center" v-if="$getColumsFlag('BfCompanyName',checkboxVal)" prop="BfCompanyName" label="开户公司主体" min-width="320" show-overflow-tooltip />
          <el-table-column align="center" v-if="$getColumsFlag('RelationNumber',checkboxVal)" prop="RelationNumber" min-width="120" label="所属客户数量">
          ...
          <el-table-column align="center" fixed="right" label="操作" min-width="55">
            <template slot-scope="scope">
...
export default {
  ...
  data () {
    return {
      ...
      formTheadOptions: [
        ...
        { label: "开户公司主体", prop: "BfCompanyName" },
        { label: "所属客户数量", prop: "RelationNumber" },
  ...
  methods: {
    ...
    UploadStatus (data, str) {
      ...
      if (data.Status == '1') {
        param.Status = '0'
        strPrompt = '你确定禁用此社保账户吗?'
        successPrompt = '禁用社保账户成功！'
      } else if (data.Status == '0') {
        param.Status = '1'
        strPrompt = '你确定启用此社保账户吗?'
        successPrompt = '启用社保账户成功！'
      }
